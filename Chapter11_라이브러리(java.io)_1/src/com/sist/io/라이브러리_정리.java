...
/*	java.io 
 * 	--------
 * 	ㄴ특징 : 데이터 이동 통로를 만들어서 데이터 입출력을 한다
 * 		 	------------
 * 			ㄴ 스트림 : 단점(단방형)
 * 						ㄴ 읽기 / 쓰기를 따로 만들어서 처리 => 네트워크는 읽기 / 쓰기를 동시에 처리(쓰레드)
 * 														-----------------------------------
 ...
 * 				  외부 라이브러리에서 주로 사용 : 보안
 * 				  ------------
 * 				  ㄴ데이터베이스 관련 (MyBatis, JPA, Spring)
 * 					*** 필수적으로 사용 : 스프링부트 (환경 설정)
 * 										ㄴproperties 파일 / ym1 파일을 주로 이용
 * 
 * 		작성법
 * 		변수 = 값
 ...
 * 			3. 관리하는 클래스가 많은 경우
 * 			   ---------------------
 * 				new 메모리 누수현상 => 한 개의 메모리만 사용이 가능하게 만든다 
 * 								   -------------------------------- 싱글턴
 * 																	----- Map
 * 		 => 컬렉션
 * 			Collection
