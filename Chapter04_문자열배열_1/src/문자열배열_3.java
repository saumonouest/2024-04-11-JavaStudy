...
public class 문자열배열_3 {
	...
	public static void main(String[] args) {
		...
		String fd=scan.next();
		for(String f:과일) {
			if(f.contains(fd)) { //startsWith도 가능 지금은
				System.out.println("영어는 "+f.replaceAll("[^A-Za-z]", "")); //알파벳을 제외하고 공백으로 대체하라는 뜻임
				/*
				 *  replaceAll : 정규식 => 문자열의 패턴
				 *  [A-Za-z] => 알파벳 전체라는 뜻 [가-힣] [0-9]
				 *  [^A-Za-z] => 알파벳을 제외하고 
				 *  ^[A-Za-z] => 알파벳으로 시작하는
				 *  [A-Za-z]$ => 알파벳으로 끝나는
				 *  
		...
		int max=0;
		int min=100; //0부터 시작하면 가장 짧은 거니까 0부터 시작하면 안 됨 max도 마찬가지 
		for(String ef:영어과일명) {
			if(max<ef.length())
				max=ef.length();
			if(min>ef.length());
		...
		for(String ef:영어과일명) {
			if(ef.length()==max) {
				System.out.println("가장 긴 단어:"+ef);
			}
			if(ef.length()==min) {
				System.out.println("가장 짧은 단어:"+ef);
			}
		}
		// 영문 단어 중 가장 짧은 단어
	}
