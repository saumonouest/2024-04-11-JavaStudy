 ...
 * 			--------------------------- 화면 UI, 웹(데이터베이스 연결, 자동 로그인, 쿠키 읽기, 권한부여)
 * 			생성자
 * 			----
 * 			ㄴ여러개 사용이 가능, 없는 경우(자동으로 컴파일러가 첨부 = 매개변수가 없는 생성자 = 디폴트 생성자)
 * 			=> 상속의 예외 조건
 * 			=> 클래스명과 동일
 * 			=> 리턴형이 없다
 * 			=> 객체 생성시에 호출되는 메소드
 ...
 * 						|		   ----------- 사용자가 요청한 내용 (id, pw, 검색어)
 * 						|			=> 입력 / 동작
 * 						|					----- 마우스 클릭, 버튼 클릭. . .
 * 					    ㄴ-- 사용자 요청 처리 결과 => 목록 / 상세보기 / 총페이지 . . .
 * 							 출력 형태 : 목록 / 상세보기
 * 									   []	클래스 
 * 				{
 * 					구현
 ...
 * 				=> 반복 제거(반복전인 코딩)
 * 						   ---------- 반복문 가능 => 메소드 => 코드의 간소화 
 * 				=> 관련된 명령문을 모아서 관리 => 가독성이 좋음(구조화된 프로그램) => 기능 분리
 * 				   --------- 입력 / 출력 ... 
 * 				=> 재사용이 좋음 (수정이나 추가가 편리)
 * 
 * 		=> 호출
 ...
 * 													  ------- 메뉴 
 * 								   ------------------------------ 요구사항 분석
 * 			어떤 데이터를 사용자에게 보여줄 지 / 데이터를 보여주기 위해 어떤 기능을 설정할 지
 * 			시작과 동시에 어떤 화면을 보여줄지
 * 
 * 
 */	
