...
import com.sist.vo.*; // Movie 클래스를 가져온 거임
 ...
 *  class A 
 *  {
 *  	int a; new 사용시에 저장 => 각 공간을 생성 후에 사용이 가능
 *  	static int b; 
 */
import java.util.*;
public class MovieManager {
	// 데이터를 고정해서 공유할 때 static 변수 사용
	// 크롤링해서 저장 => 데이터의 모음
	public static Movie [] movies = new Movie[1938]; // 자동으로 메모리에 저장 => new가 필요 없다 => 생성자 필요 없다
	/* static = 공유한다는 뜻
	 * static 은 변경이 없는 경우 => 고정 (모든 사용자 같은 내용을 보고 있는 경우) => 공유
	 * 데이터가 여러개일 때 => 메모리 공간을 한 개만 이용해서 접근 . . .
	 * 인스턴스 변수는 각각 따로 저장 (new) => 보안이 많이 필요 => private를 이용해서 저장 => 은닉화
	 * 
	 * 초기화
	...
	static { // 컴파일러에 의해 자동 처리 => 호출하지 않는다(생성자는 호출해야 된다)
		...
		try {
			...
			int i=0; //read() => char 를 읽지 않고 ASC를 읽어온다 => 문자 번호를 읽어온다
			// A => 65 => i 는 읽어온 정수를 저장할 변수 => char 로 변환
	...
	static Movie[] getMovieData(int page){	
		...
		for(int i=0;i<movies.length;i++) {
			if(j<20 && i>=pagecnt) {
				movie[j]=new Movie();
				movie[j]=movies[i]; // movie 배열에 movies에 저장된 내용 채워주는 거
				j++;
			}
		}
		return movie;
