 ...
 *	프로그램 형식
 *	1) 평문 : 순차적으로 처리 => 연산자
 *	2) 조건문(선택문) => 필요에 따라 건너뛴다
 *		=> 로그인, 아이디/비밀번호 찾기, 검색, 상세보기
 *		=> 종류 
 *			1. 단일 조건문
 *				=> 형식
 ...
 *					}
 *					=> 부정연산자(!) => 반대 효과
 *					=> 논리연산자 => 조건이 2개인 경우 ------- id가 같고 비밀번호가 같은 경우 => &&. ||
 *					=> 비교연산자 (==, !=, <, >, <=, >=)
 *					=> 프로그램 : 라이브러리 사용 / 데이터베이스(오라클) => 데이터 처리
 *					-------------------------------------------------------------------------
 *						사용자가 요청한 데이터만 읽기 => 출력
 *						--------------------
 *						ㄴ 조건 => 사용자 요청 / 처리 결과물 => 사용자 중심의 프로그램
 *						ㄴ 요구사항 분석		=>	 필요한 데이터 저장 (데이터베이스) => 화면 UI 	  =>   구현(자바)  =>  테스트(맞게 구현이 됐는지) => 배포
 *						  ---------- 아키텍처   ------------DBA			-------퍼블리셔  -----웹프로그래머 -----테스터
 *					Spring / Spring-Boot / MyBatis / JPA / JSP / MVC => 자바 기반
 *					ASP, net => C#
 *					데이터베이스(데이터 저장) => 오라클
 *
 *			2. 선택 조건문 => true / false 를 나눠서 처리
...
public class 제어문_조건문_1 {
	...
	public static void main(String[] args) {
		...
		//for(int i=1;i<=10000;i++); // i를 만 개 뽑는 문장임 여러개 안 써도 됨
		/*
		 * int a=100; int b=20; int max=0; int min=100;
