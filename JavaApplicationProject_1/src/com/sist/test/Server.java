...
public class Server implements Runnable {
	...
	// 저장 공간 (접속자)
	private Vector<Client> waitVector = new Vector<Client>();
	...
	// 접속시에 접속자 정보 저장 => 접속자마자 통신이 따로 수행 => 쓰레드
	@Override
	public void run() {
		// TODO Auto-generated method stub
		// 접속 시에 처리 
	 ...
	 *		= 보완 => 인터페이스 ***
	 *		= 내부 클래스
	 *		  -------- 클래스 메모리 할당 없이 모든 변수 / 메소드에 접근이 가능하게 만든다~
	 *			1) 멤버 클래스 : 네트워크, 쓰레드 ... 
	 *				class A {
	 *					데이터
	 *					class B { 
				...
				try {
					...
					switch(protocol) {
						...
						case Function.LOGIN:{
							...
							messageAll(Function.CHAT+"|<☞ 알림> "+name+" 입장");
							// 서버에 저장
							waitVector.add(this);
