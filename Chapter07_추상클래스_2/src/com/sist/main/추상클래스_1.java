 ...
 * 		 ---------------------------
 * 			변수
 * 			=> 따로 저장 변수 : 인스턴스 변수 => 새로운 메모리 공간을 만들어서 저장 => new 이용
 ...
 * 								----------- 생략할 수 없는 내용
 * 											--------------
 * 											상속을 내리는 클래스의 생성자가 매재변수를 가지고 있는 경우에는 생략 할 수 없다
 * 							}
 * 						} 
 ...
 * 					
 * 					5. 예외 처리가 생략 => 자바의 모든 클래스는 예외처리를 가지고 있다
 * 					   ------
 * 						ㄴ에러방지 => 에러(소스상에서 수정할 수 없는 에러) / 예외처리 (소스상에서 수정이 가능한 에러)
 * 									메모리 할당할 공간이 없다 			상수 => 문자열
 * 						  -------------------------------------------------------------------
 * 							|
 * 						컴파일시 처리, 실행시(생략이 가능)
 * 						---------  ----
 * 						javac		java - 사용자의 입력 => 처리
 * 						ㄴ파일 출력시, 네트워크, 쓰레드, 데이터베이스, 웹 . . . 
 * 		 ---------------------------
 * 		=>	기능처리 (사용자 요청 처리 ) -<  메소드,
 ...
 * 		}
 * 		 ---------------------------
 * 	추상 클래스 : 사용자 정의는 거의 없음 => 확장된 개념(인터페이스)
 * 				=> 라이브러리에 추상 클래스가 많이 존재
 * 				   -------- 상속 => 왜 오버라이딩을 하는 이유 
 * 							------------------------- 
 * 							메모리 할당을 할 수 없다 => 미완성 클래스 => 메소드 구현이 안 된 경우
 ...
 * 		-------------------------------------
 *		구현이 안 된 메소드 : 상속받은 클래스마다 기능은 동일한데 구현이 다를 경우 => 선언만
 *		-------------- 추상 메소드
 ...
 * 		}
 * 	
 * 		=> 주로 사용되는 곳은 클래스마다 기능은 동일한데 => 구현 내용이 틀린 경우 => 선언 => 유지보수가 편리하게 만든다
 * 		=> 사이트마다 약간씩 다름
 * 		=> 상속이 안 되는 소스
 * 		   --------------
