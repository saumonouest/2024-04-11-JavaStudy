 ...
 *
 *		2) 동작 순서
 *			(1) > (2) -- 여기서 false 가 되면 for문 종료
 *			for(초기값(1);조건식(2);증가식(3))
 *			{
 *				반복 수행 문장(4) -- 여기서 true 면 반복문장 수행 --> 증가식(3) --> (2) 
 *			}
 *			(1) > (2) > (4) > (3) > (2) > (4) > (3) > (2) > (4) > (3)
 *													  --------------- (2)가 false면 종료		
 *
...
public class 제어문_반복문_1 {
	...
	public static void main(String[] args) {
		...
		/*
		 * for(int i=1;i<=10;i+=2); //증가식은 프로그램에 따라 달라진다, 무조건 i++은 아님 {
		 * System.out.println(i+":Hello Java"); }
		 */
		for(int i=10;i>=1;i--) //초기값은 첫번째 값, 마지막 값, 증감식 => --, ++ //이거는 조금 그거임 조회 목록~ 에 최근 조회순으로 되어있을 때
		{
			System.out.println(i+":Hello Java");
		}
