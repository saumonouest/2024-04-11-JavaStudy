...
public class BoardMain extends JFrame implements ActionListener, MouseListener{
	...
	// 클래스 선언 => 관련있는 클래스별로 선언
	// 프로그램에 필요한 변수 설정
	int curpage=1;
	int totalpage=0;
	
	BoardSystem bs=new BoardSystem(); // 파일에 List가 저장됨
	public BoardMain() {
		setLayout(card);
		...
		bUpdate.b2.addActionListener(this);//삭제
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(bList.inBtn==e.getSource()) { // 새글 버튼 클릭
			...
			int a=JOptionPane.showConfirmDialog(this, "삭제?","삭제",JOptionPane.YES_NO_OPTION);
			...
			if(subject.length()<1) { // 입력이 안 된 경우
				bUpdate.subTf.requestFocus();
				return;
			}
			String content=bUpdate.ta.getText();
			if(content.length()<1) { // 입력이 안 된 경우
				bUpdate.ta.requestFocus();
				return;
			}
			String pw=String.valueOf(bUpdate.pwPF.getPassword());
			if(pw.length()<1) { // 입력이 안 된 경우
				bUpdate.pwPF.requestFocus();
				return;
			}
		...
		else if(bDetail.b1==e.getSource()) { // 수정
			String no=bDetail.no.getText();
			...
			String pw=String.valueOf(bInsert.pwPF.getPassword());
			if(pw.length()<1) { // 입력이 안 된 경우
				bInsert.pwPF.requestFocus();
				return;
			}
			Board b=new Board();
			b.setName(name);
